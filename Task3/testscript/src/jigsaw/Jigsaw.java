...
public class Jigsaw {
	 ...
	 * @param eNode - 目标状态节点
	 */
	public Jigsaw(JigsawNode bNode, JigsawNode eNode, int[] factors) {
	...
	public boolean BFSearch() throws IOException {
		...
		while (!openList.isEmpty()) {
			...
			if (currentJNode.equals(endJNode) && isCompleted == false) {
				isCompleted = true;
				this.calSolutionPath();
				solutionPath.add(0, currentJNode);
				break;
			}
    ...
    private void estimateValue(JigsawNode jnode) {
        int s = dislocationNum(jnode) * factors[0]
                + 1*distanceSum(jnode) * factors[1]
                + 1*adjacentDislocatedNum(jnode) * factors[2];
        jnode.setEstimatedValue(s);
    }

    // 方格内错误数码的总数
    private int dislocationNum(JigsawNode jnode) {
        int s = 0;
    ...
    // 当前错误状态与目标状态各数码的几何距离之和
    private int distanceSum(JigsawNode jnode) {
        int s = 0;
            ...
            if (currentNodeState[i] != 0 && currentNodeState[i] != endNodeState[i])
            {
                ...
                for (int j = 0; j <= dimension * dimension; j++)
                {
                    if (currentNodeState[i] == endNodeState[j])
                    {
                        row1 = (j - 1) / dimension;
                        col1 = (j + 4) % dimension;
                        s += (Math.abs(row1 - row0) + Math.abs(col1 - col0));
                        break;
                    }
                }
            }
    ...
    // 后续节点不正确的数码个数
    private int adjacentDislocatedNum(JigsawNode jnode) {
        int s = 0; 
        int dimension = JigsawNode.getDimension();
